...
def wikitq_match_func(result, target_values, strategy="top"):
    results = result #sample["chain"][-1]["parameter_and_conf"]
...
def wikitq_match_func_for_samples(all_samples, strategy="top", tagged_dataset_path='./tagged_data'):

    target_values_map = {}
    tagged_dataset_path ="./tagged_data"
    ...
    for sample in all_samples:
        try:        
            if wikitq_match_func(sample[1], target_values_map[sample[0]], strategy):
            ...
            print(f"Error : {e}")
            continue
    return sum(correct_list) / len(correct_list) , errors
